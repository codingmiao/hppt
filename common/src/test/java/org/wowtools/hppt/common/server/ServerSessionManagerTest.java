...
public class ServerSessionManagerTest {
    ...
    public static void main(String[] args) throws Exception{
        ...
        new Thread(()->{
            ServerSession session = serverSessionManager.createServerSession("localhost", 10001, 1);
            try {
                Thread.sleep(5000);
        ...
        new Thread(()->{
            ServerSession session = serverSessionManager.createServerSession("localhost", 10001, 2);
            try {
                Thread.sleep(5000);
            } catch (InterruptedException e) {
                throw new RuntimeException(e);
        ...
        new Thread(()->{
            ServerSession session = serverSessionManager.createServerSession("localhost", 10002, 3);
        }).start();
    }
}
